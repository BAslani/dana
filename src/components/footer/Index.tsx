...
export default function Footer() {
  return (
    <footer className='bg-footer sm:bg-footer-sm md:bg-footer-md lg:bg-footer-lg bg-main-background'>
      <section
        className={classNames(
          ...
          'lg:max-w-screen-lg px-[40px] md:px-[140px] lg:px-[240px]'
        )}
      >
          ...
          <div className='flex items-center gap-[16px]'>
            {socialMedias.map(({ icon, link, alt }, index) => (
              <Link href={link} key={index}>
                <Image src={icon} alt={alt} width={24} height={24} className='lg:size-[30px]' />
              </Link>
            ))}
          </div>
        </div>
        ...
        <div className='flex flex-col items-center justify-center  sm:items-stretch'>
          <div className='grid grid-cols-2 grid-rows-3 gap-x-[40px] gap-y-[20px] pl-4 sm:gap-x-[48px] sm:pl-0 lg:gap-x-[120px]'>
            {footerLinks.map(({ link, name }, index) => (
              <Link scroll={false} href={link} key={index} className='text-body-M-3 md:text-body-L-3 lg:text-body-XL-3'>
                {name}
              </Link>
            ))}
          </div>
      ...
      <div className='pb-[40px] pt-[15px] text-center text-gray-400'>&copy; 2024 Vizhen All right reserved.</div>
    </footer>
  )
}
